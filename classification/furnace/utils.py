...
# # for CAE v2
# import clip.modeling_biomedclip as BIOMED_CLIP
            ...
            if i % print_freq == 0 or i == len(iterable) - 1:
                ...
                if torch.cuda.is_available():
                    print(log_msg.format(
                        ...
                        memory=torch.cuda.max_memory_allocated() / MB), flush=True, file=sys.stderr)
                else:
                    print(log_msg.format(
                        i, len(iterable), eta=eta_string,
                        meters=str(self),
                        time=str(iter_time), data=str(data_time)), flush=True, file=sys.stderr)
            i += 1
            end = time.time()
        total_time = time.time() - start_time
        total_time_str = str(datetime.timedelta(seconds=int(total_time)))
        print('{} Total time: {} ({:.4f} s / it)'.format(
            header, total_time_str, total_time / len(iterable)), file=sys.stderr)
...
def create_d_vae(weight_path, d_vae_type, image_size, device, args=None):
    ...
    if d_vae_type == "vqgan_gumbel_f8_8192":
    ...
    elif d_vae_type == "monet":
        model = get_monet(weight_path, image_size, device, args)
        # Load the state dictionary from the URL, mapping it to the CPU
        # state_dict = torch.hub.load_state_dict_from_url(
        #     "https://aimslab.cs.washington.edu/MONET/weight_clip.pt",
        #     map_location=torch.device('cpu')
        # )
        # # Remove the CUDA device information from the state dictionary
        # for key in list(state_dict.keys()):
        #     if key.startswith('module.'):
        #         state_dict[key[len('module.'):]] = state_dict[key]
        #         del state_dict[key]
        # # model.visual.positional_embedding = torch.nn.Parameter(torch.randn(257, 1024))
        ...
        # model.eval()
        return model
    elif d_vae_type == 'biomedclip_base16':
        from open_clip import create_model_from_pretrained, get_tokenizer
        # import clip.modeling_biomedclip as BIOMED_CLIP
        # model, _ = create_model_from_pretrained('hf-hub:microsoft/BiomedCLIP-PubMedBERT_256-vit_base_patch16_224')
        model, _ = create_model_from_pretrained('hf-hub:laion/CLIP-ViT-L-14-laion2B-s32B-b82K')
        print("======Successfully load Biomed CLIP=============")
        model.to(device)
        return model
    elif d_vae_type == 'eva-clip-l14':
        from open_clip import create_model_from_pretrained, get_tokenizer
        # model = timm.create_model('eva02_large_patch14_clip_224.merged2b', pretrained=True)
        model, _, _ = open_clip.create_model_and_transforms('hf-hub:timm/eva02_large_patch14_clip_224.merged2b_s4b_b131k')
        return model
    else:
...
def get_monet(weight_path, image_size, device, args):
    # weight_path = os.path.join(weight_path, 'ViT-B-16.pt')
    model, _ = CLIP.load_monet(weight_path, device, image_resolution=image_size)
    return model

def get_biomedclip( image_size, device, args):
    model, _ = BIOMED_CLIP.load( device, image_resolution=image_size)
    return model
